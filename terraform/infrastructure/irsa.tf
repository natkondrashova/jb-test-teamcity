locals {
  irsa_maps = [
    {
      #For efs-csi dynamic provisioner
      ns         = "efs"
      sa         = "efs-csi-controller-sa"
...
resource "aws_iam_role" "irsa_iam_role" {
  ...
  name     = "${module.eks_cluster.cluster_name}-${each.value.ns}-${each.value.sa}"
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "Federated": "${module.eks_cluster.cluster_oidc_provider_arn}"
      },
...
resource "aws_iam_role_policy_attachment" "irsa_iam_policy_attachment" {
  for_each   = { for irsa_map in local.irsa_maps : "${irsa_map.ns}-${irsa_map.sa}" => irsa_map }
  role       = "${module.eks_cluster.cluster_name}-${each.value.ns}-${each.value.sa}"
  policy_arn = each.value.policy_arn
  depends_on = [aws_iam_role.irsa_iam_role]
}

resource "aws_iam_policy" "efs_controller" {
  name        = "${module.eks_cluster.cluster_name}-efs-controller"
...
resource "aws_iam_policy" "external_dns" {
  name        = "${module.eks_cluster.cluster_name}-external-dns"
  description = "Route53 policy for external-dns"
...
resource "aws_iam_policy" "cluster_autoscaler" {
  name_prefix = "eks-worker-autoscaling-${module.eks_cluster.cluster_name}"
  description = "EKS worker node autoscaling policy for cluster ${module.eks_cluster.cluster_name}"
...
data "aws_iam_policy_document" "cluster_autoscaler" {
  ...
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "autoscaling:ResourceTag/kubernetes.io/cluster/${module.eks_cluster.cluster_name}"
      values   = ["owned"]
    }
